...
class board_state {
    ...
    doMove(move, is_fast_move) {
        if (this.state[move] != 0) {
            ...
            this.pid = 3 - this.pid;
            this.last2_move = this.last_move;
            this.last_move = move;
            return true;
        }
    }
    ...
    getResult(player_id) {
        ...
        var s_cnt = 0;
        if (this.getNoneMoves().length == 0) return 0.1;  //和棋
        for (var i = 0, len = moves.length; i < len; i++) {
            ...
            //斜左下
            s_cnt = 1;
            if ((pos.row <= this.sz - this.n) && (pos.col >= this.n - 1)) {
                for (var s = 1; s < this.n; s++)
                    if (this.state[move] == this.state[move + s * this.sz - s]) s_cnt++;
                //if (s_cnt == this.n) return this.state[move];
                if (s_cnt == this.n) {
    ...
    getMoveScore(move, pid) {
        ...
        for (var s = 1; s < this.n; s++)
            if (pos.col + s < this.sz) {    //如果足够向后扩展，下同
                if (sim_state[move + s] != rival_pid) //只要不是对方棋子则继续判断，下同
                    l_u.set('f', s, (sim_state[move + s] == pid) ? 1 : 0)  //若相同则记1，否则记0，下同
            } else break;
        //九点
        for (var s = 1; s < this.n; s++)
            if (pos.col - s >= 0) {
                if (sim_state[move - s] != rival_pid)
                    l_u.set('b', s, (sim_state[move - s] == pid) ? 1 : 0)
            } else break;
        move_score += l_u.calcScore();

        //六点
        l_u.reset();
        for (var s = 1; s < this.n; s++)
            if (pos.row + s < this.sz) {
                if (sim_state[move + s * this.sz] != rival_pid)
        ...
        for (var s = 1; s < this.n; s++)
            if (pos.col + s < this.sz && pos.row - s >= 0) {
                if (sim_state[move - s * this.sz + s] != rival_pid)
    ...
    getMaxScoreMove(moves, return_type, if_reverse) {   //return_type: 0-返回最高move及分值，1-返回所有分值，2-返回排序后的move
        ...
        var rival_move_score = 0;
        var mapped_moves;
        if (moves.length == 0) return null;   //如果没有可走的棋，则返回null

        for (var i = 0, len = moves.length; i < len; i++) {   //每个可走的位置都遍历一遍
            var move = moves[i];
            moves_score[move] = this.getMoveScore(move, this.pid);
            rival_move_score = this.getMoveScore(move, 3 - this.pid);
            //moves_score[move] = rival_move_score > 6000 ? moves_score[move] + Math.floor(rival_move_score / 2) : moves_score[move]
            moves_score[move] += Math.floor(rival_move_score / 4)
        }
        ...
        switch (return_type) {
            case 0: {
                ...
                if (maxVal == 0) return {   //如果最大值为0，则在可走的棋中随机返回一个
                    idx: moves[Math.floor(Math.random() * moves.length)],    //在可走的棋中随机返回一个
                    val: maxVal
                }
                if (idxs != -1) return {
...
class line_unit {
    constructor(unit_len) {
        ...
        this.unit = new Array(this.line_len).fill(2);   //2表示对方，1表示自己，0表示空
        this.weight = [1, 3, 5, 3, 1];
    }
