...
class board_state {
    ...
    getMaxScoreMove(moves, return_type) {   //return_type: 0-返回最高move，1-返回所有分值，2-返回排序后的move
        ...
        for (var i = 0, len = moves.length; i < len; i++) {   //每个可走的位置都遍历一遍
            ...
            l_u.reset();
            for (var s = 1; s < this.n; s++)
                if (pos.col + s < this.sz) {
                    if (sim_state[move + s] != 3 - pid)
                        l_u.set('f', s, (sim_state[move + s] == this.pid) ? 1 : 0)
                } else break;
            //九点
            for (var s = 1; s < this.n; s++)
                if (pos.col - s >= 0) {
                    if (sim_state[move - s] != 3 - pid)
                        l_u.set('b', s, (sim_state[move - s] == this.pid) ? 1 : 0)
                } else break;
            ...
            for (var s = 1; s < this.n; s++)
                if (pos.row + s < this.sz) {
                    if (sim_state[move + s * this.sz] != 3 - pid)
                        l_u.set('f', s, (sim_state[move + s * this.sz] == this.pid) ? 1 : 0);
                } else break;
            //十二点
            for (var s = 1; s < this.n; s++)
                if (pos.row - s >= 0) {
                    if (sim_state[move - s * this.sz] != 3 - pid)
                        l_u.set('b', s, (sim_state[move - s * this.sz] == this.pid) ? 1 : 0);
                } else break;
            moves_score[move] += l_u.calcScore();
            ...
            for (var s = 1; s < this.n; s++)
                if (pos.col + s < this.sz && pos.row - s >= 0) {
                    if (sim_state[move - s * this.sz + s] != 3 - pid)
                        l_u.set('f', s, (sim_state[move - s * this.sz + s] == this.pid) ? 1 : 0);
                } else break;
            //七点半
            for (var s = 1; s < this.n; s++)
                if (pos.col - s >= 0 && pos.row + s < this.sz) {
                    if (sim_state[move + s * this.sz - s] != 3 - pid)
                        l_u.set('b', s, (sim_state[move + s * this.sz - s] == this.pid) ? 1 : 0);
                } else break;
            moves_score[move] += l_u.calcScore();
