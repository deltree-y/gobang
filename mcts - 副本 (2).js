    ...
    for (var i = 0; i < itermax; i++) {
        ...
        if (node.untriedMoves.length != 0) {
            ...
            if (is_selected == true || i == 0)
                var best_moves_sorted = state.getMaxScoreMove(state.getNoneMoves(), 2, false);
        ...
        var best_move = state.getMaxScoreMove(state.getNoneMoves(), 0, false);
        var best_move_rival = state.getMaxScoreMove(state.getNoneMoves(), 0, true);
        if (best_move != null && best_move_rival.val >= 1000 && best_move_rival.val > best_move.val)
            best_move = best_move_rival;
        while (best_move != null) {
            ...
            if (rlt != 0)
                break;
            best_move = state.getMaxScoreMove(state.getNoneMoves(), 0, false);
            best_move_rival = state.getMaxScoreMove(state.getNoneMoves(), 0, true);
            if (best_move != null && best_move_rival.val >= 1000 && best_move_rival.val > best_move.val)
                best_move = best_move_rival;
